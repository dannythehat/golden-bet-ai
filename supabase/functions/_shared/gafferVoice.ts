/**
 * THE GAFFER — voice engine.  [EDGE MIRROR]
 *
 * ⚠ Deno/edge mirror of src/lib/gafferVoice.ts (the frontend holds the canonical
 * copy). Keep the two in sync — same banks, same logic — so the LIVE picks
 * (gaffer-daily-pick) speak in exactly the same voice as the site's local
 * fallback. Only the import path differs (Deno needs the explicit .ts extension).
 *
 ...
 * assembled from the phrase banks (./gafferPhrases.ts) and an anti-repeat memory
 * ensures nothing recently used comes back round.
 *
 * Architecture (locked): football engine → signals → Gaffer voice → fresh words.
 * See docs/gaffer/03_LANGUAGE.md, 04_HUMOUR.md, 10_PHRASE_MEMORY.md.
 */
import {
  OPENERS, MARKET_FLAVOUR, VERDICT_STRONG, VERDICT_VALUE, EDGE_PHRASES,
  HEDGES, SIGN_OFFS, ASIDES, NO_BET, DONKEY_ROASTS, nickname,
} from './gafferPhrases.ts';

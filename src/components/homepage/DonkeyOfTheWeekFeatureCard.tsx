...
/** Donkey of the Week — fully data-driven from homepage_donkey. */
export function DonkeyOfTheWeekFeatureCard() {
  ...
  const ctaLabel = donkey?.cta_label ?? 'Could it be you?';
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
        {/* Image / chalkboard */}
        {donkey?.image ? (
          <div className="overflow-hidden rounded-2xl border border-fuchsia-400/20 bg-black/40">
            <img src={donkey.image} alt={headline} className="h-full max-h-80 w-full object-cover" loading="lazy" />
          </div>
        ) : (
        ...
        <div className="space-y-4 rounded-2xl border border-fuchsia-400/20 bg-white/[0.03] p-6">
          ...
          <a href={ctaHref} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-amber-400 to-yellow-500 px-6 py-3.5 font-display text-xl tracking-wide text-[#160a02] shadow-[0_10px_30px_-12px_rgba(250,204,21,0.9)] transition-transform hover:scale-[1.01]">
            {ctaLabel.toUpperCase()} <ArrowRight className="h-5 w-5" />
          </a>
        </div>
      </div>
    </SectionShell>
